import type { Event } from './types';

export const event: Event = {
  id: "lecture-2026-04-14",
  date: "2026-04-14",
  year: "2026",
  type: "Lecture",
  image: "/images/events/2026/lecture-2026-04-14.jpg",
  title_zh: "全球化地緣政治知識的新局面：強人（政治）與主權的再詮釋",
  title_en: "The New Politics of Globalizing Geopolitical Knowledge: Rearticulating Strongmen & Sovereignty",
  content_zh: `
本所「歐洲聯盟重點研究計畫」及「AI治理觀念實驗室」邀請Michael Kennedy教授 (Sociology and International and Public Affairs Studies, Brown University) 於民國115年4月14日（星期二）上午10時30分 於本所研究大樓一樓會議室舉行演講，講題為 “The New Politics of Globalizing Geopolitical Knowledge: Rearticulating Strongmen & Sovereignty (全球化地緣政治知識的新局面：強人（政治）與主權的再詮釋)”，歡迎踴躍參加。

主講人：Michael Kennedy教授（Sociology and International and Public Affairs Studies, Brown University）
與談人：邵允鍾（中央研究院歐美所助研究員）
主持人：何之行（中央研究院歐美所副研究員）

大綱: 過去十年的發展，在多大程度上強化或挑戰了Prof. Kennedy於2015年出版的《知識全球化》一書所提出的問題？回顧這十年，我們可以看到，人們對學術責任與大學問責的關注仍然存在。然而，美國與全球權力關係正在改變—俄羅斯入侵烏克蘭是一個明顯的例子，而強人政治在其他地區的擴散也反映了這一趨勢。這些變化正在挑戰我們對學術自由的理解，以及對多元與包容政治的看法。Kennedy以他在布朗大學的經驗為例，回顧近年的發展，並思考未來可能的方向。

  `,
  content_en: `
  Speaker: Prof Michael Kennedy (Sociology and International and Public Affairs Studies, Brown University)

Panlist: Dr. Yung-Djong Shaw (Assistant Research Fellow, Institute of European and American Studies, Academia Sinica)

Moderator: Dr. Chih-hsing Ho (Associate Research Fellow, Institute of European and American Studies, Academia Sinica)

Time: 10:30 A.M. ~ 12:00 P.M. Tuesday, April 14, 2026.

Venue: 1st Floor Conference Room, Institute of European and American Studies, Academia Sinica

Abstract: How has the last decade amplified and challenged the themes of Kennedy’s 2015 book, Globalizing Knowledge? First discussed publicly at Singapore Management University in that year, we can see that concerns over intellectual responsibility and university accountability abide. However, changes in American and global power relations, anticipated by Russia’s invasion of Ukraine but finding new fulfillment in the proliferation of strongmen politics elsewhere, challenge how academic freedom is to be conceived and the politics of diversity and inclusion articulated. Kennedy draws on his own Brown University’s experience to characterize the recent past & anticipate alternative futures.

`,

  speakers: [
    { name_zh: "Michael Kennedy", name_en: "Michael Kennedy", affiliation_zh: "布朗大學社會學暨國際與公共事務系", affiliation_en: "Sociology and International and Public Affairs Studies, Brown University" },
    { name_zh: "邵允鍾", name_en: "Yung-Djong Shaw", affiliation_zh: "中央研究院歐美研究所", affiliation_en: "Institute of European and American Studies, Academia Sinica" },
    { member: "chih-hsing-ho" },
  ],
};

export default event;
